...
pub fn setup(app: &AppWindow, app_handle: slint::Weak<AppWindow>) {
    let custom_window_chrome = !app.get_use_native_window_chrome();

    let ah = app_handle.clone();
    app.on_drag_window(move || {
        if custom_window_chrome && let Some(_app) = ah.upgrade() {
            #[cfg(not(target_os = "macos"))]
            {
    ...
    app.on_toggle_maximize_window(move || {
        if custom_window_chrome && let Some(app) = ah.upgrade() {
